...
#SBATCH --job-name=age11mtemp
#SBATCH --output=mri-template-age11m-creation-%j.out
#SBATCH --error=mri-template-age11m-creation-%j.err
...
UNIQUE_TEMP_DIR=${TMPROOT:-/tmp}/TemplateCreationAge11Male
...
apptainer exec --bind $UNIQUE_TEMP_DIR:/writabletemp \
               ...
               /bin/bash -c "source ${FSLDIR}/etc/fslconf/fsl.sh && \
		            ...
		            cd /mnt/Age11/male/intensity_improved_selected_for_template &&\
                    antsMultivariateTemplateConstruction2.sh \
                        -d 3 -o template0_ -i 12 \
                        -z /mnt/Age11/male/intensity_improved_selected_for_template/REFERENCE_SUBJECT.nii.gz -r 1 \
                        -g 0.10 -c 0 -k 1 -w 1 -y 0\
                        -f 8x4x2x1 -s 4x2x1x0vox -q 120x90x60x20 \
                        -l 0 \
                        -t "SyN[0.1]" -m "CC[2]" \
                        -n 0 -u 1 \
                        /mnt/Age11/male/intensity_improved_selected_for_template/*.nii.gz"
