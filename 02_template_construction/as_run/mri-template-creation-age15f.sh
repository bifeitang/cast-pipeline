...
#SBATCH --job-name=age15ftemp
#SBATCH --output=mri-template-age15f-creation-%j.out
#SBATCH --error=mri-template-age15f-creation-%j.err
...
UNIQUE_TEMP_DIR=${TMPROOT:-/tmp}/TemplateCreationAge15Female
...
apptainer exec --bind "$UNIQUE_TEMP_DIR":/writabletemp \
               ...
               /bin/bash -c "source ${FSLDIR}/etc/fslconf/fsl.sh && \
                    ...
                    cd /mnt/Age15/female/intensity_improved_selected_for_template && \
                    antsMultivariateTemplateConstruction2.sh \
                        -d 3 -o template0_ -i 12 \
                        -z /mnt/Age15/female/intensity_improved_selected_for_template/REFERENCE_SUBJECT.nii.gz -r 1 \
                        -g 0.10 -c 0 -k 1 -w 1 -y 0 \
                        -f 8x4x2x1 -s 4x2x1x0vox -q 120x90x60x20 \
                        -l 0 \
                        -t \"SyN[0.1]\" -m \"CC[2]\" \
                        -n 0 -u 1 \
                        /mnt/Age15/female/intensity_improved_selected_for_template/*.nii.gz"
