#!/usr/bin/env bash
#SBATCH --job-name=dc-nki
#SBATCH --output=dc-nki-%j.out
#SBATCH --error=dc-nki-%j.err
...
# Deformation cost on the CSF-normalised INTENSITY images.
#
...
# NKI templates are not sex-stratified; the 4th arg is accepted and ignored for symmetry
...
OUT_ROOT="$ROOT_DIR/DC_NKI_INTENSITY_2026-07-28"
STAGE="$ROOT_DIR/TemplateCreationTemp/${EID}_dci_${SLURM_JOB_ID:-local}"
CONTAINER_IMAGE="/project/contreras-vidal/Image/mri_template_env_reg.sif"

TPL_ID="NKI_age${TPL_AGE}"
...
TMASK_HOST="$DB_DIR/Templates/NKI_age${TPL_AGE}_brain_brain_mask.nii.gz"
TMASK_ARG=""
[[ -f "$TMASK_HOST" ]] && TMASK_ARG="--template-mask \"/mnt/Templates/NKI_age${TPL_AGE}_brain_brain_mask.nii.gz\""
