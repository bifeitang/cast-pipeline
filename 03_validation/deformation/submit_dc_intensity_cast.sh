#!/usr/bin/env bash
# Submit the CAST portion of the intensity rerun: 319 held-out subjects x 10 templates
...
# pre/post pairs).
#
# Usage: submit_dc_intensity_cast.sh <subject_age_map.tsv> [--dry-run]
...
MAP="${1:?usage: submit_dc_intensity_cast.sh <subject_age_map.tsv> [--dry-run]}"
DRY=""; [ "${2:-}" = "--dry-run" ] && DRY=1
HERE="$(cd "$(dirname "$0")" && pwd)"

TPL_AGES="7 8 9 10 11"
TPL_SEXES="male female"
...
while IFS=$'\t' read -r eid age; do
  ...
  for ta in $TPL_AGES; do
    for ts in $TPL_SEXES; do
      n_job=$((n_job + 1))
      [ -n "$DRY" ] && continue
      sbatch --parsable "${HERE}/sbatch_dc_intensity.sh" "$eid" "$age" "$ta" "$ts" >/dev/null
    done
  done
done < "$MAP"
